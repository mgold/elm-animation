...
static x =
    A <| AnimRecord 0 0 (Duration 0) Nothing defaultEase x x
...
equals (A a) (A b) =
    a.start
        ...
        && a.ramp
        == b.ramp
        && (a.dos == b.dos || 1.0e-3 >= abs (dur a.dos a.from a.to - dur b.dos b.from b.to))
...
isRunning t (A { start, delay, dos, from, to }) =
    let
        duration = dur dos from to
    in
        t > start + delay && t < start + delay + duration
